...
外部AI API、Supabase Service Role、Stripe API、Google Sheets API、Microsoft Graph APIはブラウザから直接呼ばず、必ず本APIを経由する。
...
MVPではSupabase Authを基本とする。画面側はSupabase Authのセッションを保持し、認証済みAPIはSupabase AuthのJWTまたは安全なセッションCookieを検証する。

アプリケーションDBの `users` は、Supabase Authの `auth.users.id` と紐づくプロフィール/権限テーブルとして扱う。パスワードハッシュ、パスワード再設定トークン、認証メール送信はSupabase Auth側で管理する。
...
認証処理はSupabase Authを利用する。以下のAPIは、画面実装上必要な場合のアプリケーションラッパーとして定義する。Supabase Authクライアントを直接使う場合でも、ログイン後の会社情報、ロール、権限はアプリケーションAPIで取得する。
...
- Supabase Authでメールアドレスとパスワードを検証する。
- Supabase Authの `auth.users.id` からアプリDBの `users.auth_user_id` を検索する。
- `users.status = active` であることを確認する。
- `tenants.status = active` であることを確認する。
- Supabase Authのセッションを確立する。
- `last_login_at` を更新する。
...
- パスワード再設定メールとトークン管理はSupabase Authを利用する。アプリDBには再設定トークンを保存しない。
...
- Stripe Customerは会社単位で作成し、`tenants.stripe_customer_id` に保存する。
- `tenant_id`、契約プラン、オプション情報はStripe metadataにも保持する。
- Checkout Session作成時点では機能を有効化しない。Webhookで支払い完了または契約有効化を確認してからDBを更新する。

### 6.5 Stripe Customer Portal作成
...
| イベント | 処理 |
| --- | --- |
| `checkout.session.completed` | 初回契約またはオプション申込の完了を記録する |
| `customer.subscription.created` | 契約ID、プラン、ステータスを保存する |
| `customer.subscription.updated` | プラン変更、オプション変更、支払状態を同期する |
| `customer.subscription.deleted` | 解約状態を同期し、必要に応じて機能を停止する |
| `invoice.payment_failed` | 管理者通知、猶予期間、機能制限の判定に使う |

重要ルール:

- `Stripe-Signature` を必ず検証する。
- `stripe_event_id` を保存し、同一イベントを二重処理しない。
- Webhook受信後、`tenants.subscription_plan`、`tenants.subscription_status`、`tenants.feature_flags`、課金履歴を更新する。
- アプリ内の機能制御はStripe APIを都度参照せず、同期済みDBを参照する。
...
| --- | --- |
| `q` | 品目名検索 |
| `external_item_code` | 外部品目コード完全一致 |
| `unit` | 単位 |
...
```json
{
  "data": [
    {
      "id": "uuid",
      "external_item_code": "EXT-WALL-001",
      "name": "外壁塗装",
      "unit": "m2",
      "unit_price": 2500,
      "is_active": true
...
- `external_item_code` は任意。ただし指定する場合、同一会社内で重複不可とする。
- 既存見積明細の単価は自動変更しない。
...
| --- | --- |
| `insert` | 新規追加のみ |
| `upsert` | 外部品目コードがあれば同一外部品目コードを優先して更新。外部品目コードがなければ同一品目・単位で更新 |

取り込みルール:

- `external_item_code` は任意項目とする。
- `external_item_code` が指定されている行は、同一会社内で重複不可とする。
- `upsert` 時は `external_item_code` 完全一致を最優先し、品目名・単位一致より優先する。
- 外部品目コードが重複している場合はエラー行として扱い、自動取り込みしない。
...
{
  "price_item_id": "uuid",
  "external_line_id": "EXT-LINE-0001",
  "external_item_code": "EXT-WALL-001",
  "location": "外壁",
  "item_name": "外壁塗装",
  "description": "下塗り・上塗り",
...
- `external_item_code` が指定され、`price_item_id` が未指定の場合、同一会社の `price_items.external_item_code` と完全一致する単価マスターを検索して `price_item_id` に変換する。
- `external_item_code` と `price_item_id` の両方が指定された場合は、両者が同一会社内で対応していることを検証する。不一致なら `VALIDATION_ERROR` とする。
- `external_line_id` は外部システム由来の明細IDとして保持する。同一見積内で重複する場合はエラー扱いを基本とする。
- `amount` はサーバー側で計算する。
- 追加後に見積合計を再計算する。
...
- `external_item_code` が指定され、`price_item_id` が未指定の場合、同一会社の `price_items.external_item_code` と完全一致する単価マスターを検索して `price_item_id` に変換する。
- `external_item_code` と `price_item_id` の両方が指定された場合は、両者が同一会社内で対応していることを検証する。不一致なら `VALIDATION_ERROR` とする。
- `external_line_id` は同一見積内で重複不可とする。
- `price_item_id` が変わった場合は、選択された単価マスターの `unit` と `unit_price` を初期反映する。ただしユーザーが明示的に手入力単価へ変更する操作は別途許可する。
...
- AIは顧客情報、見積ヘッダー情報、顧客向け備考を入力しない。
- AIは明細の単位を判断しない。単位と単価は、ユーザーが選択した単価マスターから取得する。
...
- AI反映APIでは、顧客情報、見積ヘッダー情報、顧客向け備考を受け取らない。
- `price_item_id` は同一会社の単価マスターのみ許可する。
- `unit` と `unit_price` はリクエスト値を採用せず、選択された `price_item_id` の単価マスターからサーバー側で取得する。
- `price_item_id` が未選択の明細は、ユーザーが単価マスター選択または手入力扱いを明示するまで確定明細にしない。
- 金額はサーバー側で再計算する。
...
1. Supabase AuthのセッションまたはJWTを検証する。
2. `users.auth_user_id` から `tenant_id`、ロール、権限を取得する。
3. 取得、更新、削除の条件に `tenant_id` を含める。
4. ID指定されたデータが他会社のものなら `404` とする。
5. Supabase Service Roleを使う処理でも、`tenant_id` 条件を省略しない。
