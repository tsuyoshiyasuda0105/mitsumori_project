...
| 見積アプリDB連携 | 見積保存API、AI候補の明細反映、単価マスターAPI/一覧DB接続は実装済み。音声以外はDB連携を本線にする | Neon `DATABASE_URL` 待ち。顧客DB接続はDB接続前に設計可能 | `web/app/api/price-items/route.ts`, `web/components/lists/PriceItemList.tsx`, `web/components/focus/AiReview.tsx`, `web/app/(main)/estimates/[id]/export/page.tsx` |
