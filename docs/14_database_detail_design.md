...
- PostgreSQL Row Level Securityを導入し、`tenant_id = current_setting('app.tenant_id')::uuid` を強制する。
...
| 1 | `tenants` | 基本 | 会社 | 必須 |
| 2 | `users` | 認証 | ユーザー | 必須 |
| 3 | `user_sessions` | 認証 | ログインセッション | 必須 |
| 4 | `password_reset_tokens` | 認証 | パスワード再設定 | 必須 |
...
ログインユーザーを管理する。ユーザーは必ず1つの会社に所属する。
...
セッション管理を行う。JWTのみで運用する場合は省略可能だが、強制ログアウトや端末管理に備えてDB管理を推奨する。

| カラム | 型 | NULL | キー | 初期値 | 説明 |
| --- | --- | --- | --- | --- | --- |
| `id` | uuid | NO | PK | `gen_random_uuid()` | セッションID |
| `tenant_id` | uuid | NO | FK |  | 会社ID |
| `user_id` | uuid | NO | FK |  | ユーザーID |
| `session_token_hash` | varchar(255) | NO | UNIQUE |  | セッショントークンハッシュ |
...
| 制約 | 内容 |
| --- | --- |
| `fk_user_sessions_users` | `FOREIGN KEY (tenant_id, user_id) REFERENCES users(tenant_id, id)` |
...
| カラム | 型 | NULL | キー | 初期値 | 説明 |
| --- | --- | --- | --- | --- | --- |
| `id` | uuid | NO | PK | `gen_random_uuid()` | トークンID |
| `tenant_id` | uuid | NO | FK |  | 会社ID |
| `user_id` | uuid | NO | FK |  | ユーザーID |
| `token_hash` | varchar(255) | NO | UNIQUE |  | トークンハッシュ |
| `expires_at` | timestamptz | NO |  |  | 有効期限 |
...
| 制約 | 内容 |
| --- | --- |
| `fk_password_reset_tokens_users` | `FOREIGN KEY (tenant_id, user_id) REFERENCES users(tenant_id, id)` |
...
MVPでは品目名の完全一致・部分一致を基本とする。将来、類似検索用に `pg_trgm` またはベクトル検索を追加できる。
...
| `schema_version` | varchar(20) | NO |  | `'1.0'` | AI解析JSONスキーマ版 |
...
音声、アップロードExcel、生成Excel、PDF、ロゴを管理する。実ファイルはS3互換ストレージに保存する。
...
APIはDB接続またはトランザクション開始時に以下を設定する。

```sql
SET LOCAL app.tenant_id = 'ログイン中のtenant_id';
```

### 10.3 注意点

- バッチ処理やWorkerでも必ず `app.tenant_id` を設定する。
...
2. DBトランザクションで `SET LOCAL app.tenant_id` を設定する。
3. 対象データを必ず `tenant_id` 付きで再取得する。
...
1. PostgreSQL拡張を有効化する。
   ...
   - 必要に応じて `pg_trgm`
2. `tenants` を作成する。
3. `users`, `user_sessions`, `password_reset_tokens` を作成する。
4. `customers`, `price_items` を作成する。
5. `files` を作成する。
6. `ai_analysis_runs` を作成する。
7. `estimates` を作成する。
8. `estimate_lines` を作成する。
9. `import_jobs`, `export_jobs` を作成する。
10. `audit_logs` を作成する。
11. 別オプション用に `meeting_recordings` を作成する。
12. インデックスを作成する。
13. RLSを有効化する。
14. 初期管理者、初期会社、標準設定を登録する。
...
- RLS有効化後、`app.tenant_id` 未設定の接続では業務データを参照できない。
