...
| バックエンド | Next.js API Routes または NestJS / TypeScript | フロントエンドと型を共有しやすく、MVP後のAPI分離にも移行しやすい。 |
| DB | PostgreSQL | 会社単位のデータ分離、JSONBによるAI解析結果保存、検索、集計に向く。 |
| ORM | Prisma | 型安全なDBアクセス、マイグレーション管理、初期開発速度に向く。 |
| ファイル保存 | S3互換オブジェクトストレージ | 音声、ロゴ、Excel、PDFなどのバイナリ保存に向く。 |
| 非同期処理 | BullMQ + Redis またはクラウドキュー | 文字起こし、AI解析、Excel/PDF生成を画面処理から分離できる。 |
| 認証 | Auth.js / 独自JWTセッション | メール・パスワード認証、将来のSSO拡張に対応しやすい。 |
...
flowchart LR
  ...
  Web --> API["アプリケーションAPI"]
  API --> DB["PostgreSQL"]
  API --> Storage["オブジェクトストレージ"]
  API --> Queue["ジョブキュー"]
  Queue --> Worker["バックグラウンドWorker"]
...
| アプリケーションAPI | 認証、権限判定、会社単位のデータ分離、DB更新、ファイル署名URL発行、ジョブ投入を担当する。 |
| Worker | 音声文字起こし、AI構造化解析、Excel取り込み検証、Excel/PDF生成など時間のかかる処理を実行する。 |
| PostgreSQL | 会社、ユーザー、顧客、単価マスター、見積、AI解析履歴、出力履歴を保存する。 |
| オブジェクトストレージ | 音声ファイル、会社ロゴ、アップロードExcel、生成Excel、生成PDFを保存する。 |
...
AI解析結果は候補であり、ユーザー確定まで見積には反映しない。スキーマはJSON Schemaとしてバージョン管理し、`schema_version` で互換性を管理する。
...
```json
{
  ...
  "estimate": {
    ...
    "site_address": "東京都...",
    "desired_date": null,
    "customer_note_draft": "外壁塗装工事一式のお見積りです。"
  },
  "line_candidates": [
    {
...
- `ai_suggested_unit_price` は参考値として扱い、初期値にはユーザーが選択した単価マスター品目の `unit_price` を優先する。
- `missing_information` は見積編集画面で確認事項として表示する。
- `internal_vendor_instruction_candidates` は内部情報候補として扱い、ユーザーが確定した場合のみ `estimates.internal_vendor_instruction` または `estimate_lines.internal_vendor_instruction` に保存する。
...
- パスワードはArgon2idまたはbcryptでハッシュ化して保存する。
- セッションCookieは `HttpOnly`, `Secure`, `SameSite=Lax` 以上を設定する。
