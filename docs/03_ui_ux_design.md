...
5. AI解析結果で顧客、現場住所、場所、品目、数量、単価候補、確認事項を確認
6. 「見積に反映」を実行
...
- 音声入力前に、短いヒントとして「場所、品目、数量、単位、備考、業者指示事項を話す」程度の入力ガイドを表示する。
...
- 解析サマリー: 顧客名、現場住所、件名案、作業概要、入力元を表示する。
- 見積明細候補: 場所、品目、数量、単位、単価候補、金額、備考、単価マスター一致度を表形式で表示する。
...
- 既存データへの追加登録か、品目と単位の一致時に上書きするかを選択する。
